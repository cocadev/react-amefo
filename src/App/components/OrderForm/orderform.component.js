...
import { getProduct, orderProduct } from "../../Auth/eventActions";
import { SERVER_URL } from "../../../config";

import "react-image-crop/dist/ReactCrop.css";
// import CardPayment from "../CardPayment/cardpayment.component";
  ...
  constructor(props) {
    ...
    if (this.props.location.search) {
      ...
      paramsArr.forEach(param => {
        if (param.split("=")[0] === "pid") {
          let productId = param.split("=")[1].split("$")[0];
          let userId = param.split("=")[1].split("$")[1];
          this.props.getProduct(productId).then(res => {
            let product = res.data.product;
            if (product) {
              this.setState({ product: product, userId: userId });
            }
          });
        }
      });
